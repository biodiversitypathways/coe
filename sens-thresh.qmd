...
decibel_dates <- dir_ls("./spl/date", regexp = ".xls") |>
  map_dfr(
    ~read_csv(.x, skip = 2) |>
      mutate(
        filename = basename(.x),                     
  ...
  mutate(location = gsub("\\.xls$", "", location))
...
coe_locations <- wt_get_sync("organization_locations","data",organization=5466)
...
decibels_filtered <- decibel_meter_data |>
  ...
  ungroup() |>
  inner_join(coe_locations |> dplyr::select(location, latitude, longitude), by = "location") |>
